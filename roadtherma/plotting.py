...
def categorical_heatmap(ax, pixels, distance, transversal_resolution, categories):
    ...
    mat = ax.imshow(pixels, aspect='auto', vmin=np.min(pixels) - .5, vmax=np.max(pixels) + .5, cmap=cmap)
    # tell the colorbar to tick at integers
    cbar = plt.colorbar(mat, ax=ax, ticks=np.arange(np.min(pixels), np.max(pixels) + 1))
...
def plot_heatmaps(titles, metadata, transversal_resolution, pixel_temperatures, pixel_category, categories):
    ...
    temperature_heatmap(ax1, pixel_temperatures, metadata.distance, transversal_resolution)
    ...
    categorical_heatmap(ax2, pixel_category, metadata.distance, transversal_resolution, categories)
    return fig_heatmaps
...
def plot_statistics(title, temperatures, roadwidths, road_pixels, tolerances):
    ...
    high_gradients = _calculate_tolerance_vs_percentage_high_gradient(temperatures, roadwidths, road_pixels, tolerances)
...
def _calculate_tolerance_vs_percentage_high_gradient(temperatures, roadwidths, road_pixels, tolerances):
    ...
    for tolerance in tolerances:
        gradient_pixels, _ = detect_high_gradient_pixels(temperatures.values, roadwidths, tolerance)
        percentage_high_gradients.append((gradient_pixels.sum() / nroad_pixels) * 100)
