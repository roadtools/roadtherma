...
def temperature_to_csv(file_path, dataset, road_pixels):
    temperatures, extra_data = split_temperature_data(dataset)
    filename = file_path.split("/")[-1]
    temperatures.values[~road_pixels] = 'NaN'
    df = merge_temperature_data(extra_data, temperatures)
    df.to_csv(f"gradient_{filename}.csv")


def detections_to_csv(file_path, dataset, road_pixels, detected_pixels):
    temperatures, extra_data = split_temperature_data(dataset)
    filename = file_path.split("/")[-1]
...
def temperature_mean_to_csv(file_path, dataset, road_pixels):
    temperatures, _extra_data = split_temperature_data(dataset)
    filename = file_path.split("/")[-1]
    temperatures.values[~road_pixels] = 'NaN'
